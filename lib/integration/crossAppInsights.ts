...
import type { UnifiedHealthData, CrossAppInsight } from "./types"
...
export class CrossAppInsightEngine {
  ...
  generateCrossAppInsights(): CrossAppInsight[] {
    ...
    return insights.sort((a, b) => b.confidence - a.confidence).slice(0, 20) // Return top 20 insights
  }
  ...
  private calculateCorrelation(type1: string, type2: string): { correlation: number; dataPoints: number } {
    const dataPoints = this.unifiedData.filter((item) => this.hasDataType(item, type1) && this.hasDataType(item, type2))
  ...
  private hasDataType(item: UnifiedHealthData, type: string): boolean {
    switch (type) {
      ...
      case "mental":
        return !!item.mental
      case "sleep":
        return !!item.sleepStress?.sleep
      case "stress":
        return !!item.sleepStress?.stress
      case "nutrition":
  ...
  private extractNumericValue(item: UnifiedHealthData, type: string): number {
    switch (type) {
      case "skin":
        return item.skin?.severity || 0
      case "gastro":
        return (item.gastro?.symptoms?.reflux || 0) + (item.gastro?.symptoms?.bloating || 0)
      case "mental":
        return item.mental?.mood || 0
      case "sleep":
        return item.sleepStress?.sleep?.quality || 0
      case "stress":
        return item.sleepStress?.stress?.level || 0
      case "nutrition":
        return item.nutrition?.stress || 0
      case "exercise":
        return item.exercise ? 1 : 0
      default:
        return 0
    }
  }
  ...
  private analyzeSymptomClusters(): Array<{ symptoms: string[]; description: string }> {
    ...
    return clusters.filter((cluster) =>
      cluster.symptoms.every((symptom) => this.unifiedData.some((item) => this.hasDataType(item, symptom))),
    )
  }
  ...
  private predictFlareUpRisk(): { risk: number; confidence: number; factors: string[] } {
    ...
    const avgStress =
      recentData.reduce((sum, item) => sum + (item.sleepStress?.stress?.level || item.mental?.stress || 0), 0) /
      recentData.length
    ...
    const avgSleepQuality =
      recentData.reduce((sum, item) => sum + (item.sleepStress?.sleep?.quality || 0), 0) / recentData.length
    ...
    const recentMood = recentData.slice(-3).reduce((sum, item) => sum + (item.mental?.mood || 0), 0) / 3
    const earlierMood = recentData.slice(-7, -3).reduce((sum, item) => sum + (item.mental?.mood || 0), 0) / 4
