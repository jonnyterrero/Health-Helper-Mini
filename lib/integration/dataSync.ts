...
export class HealthDataSync {
  ...
  private generateMockData(appId: string, dataType: string): any[] {
    const mockDataGenerators = {
      'skintrack-plus': {
        skin_condition: () => [
          { date: '2023-10-20', skinCondition: 'mild_breakout', severity: 4, affectedAreas: ['forehead'], triggers: ['stress'], treatments: ['salicylic_acid'] },
      ...
      'gastroguard': {
        symptoms: () => [
          { date: '2023-10-20', symptoms: { reflux: 6, bloating: 4, nausea: 2, stomachPain: 3, diarrhea: 0, constipation: 0 } },
          { date: '2023-10-21', symptoms: { reflux: 3, bloating: 2, nausea: 1, stomachPain: 1, diarrhea: 0, constipation: 0 } },
        ],
        meals: () => [
      ...
      'mindtrack': {
        ...
        journal: () => [
          { date: '2023-10-20', journalEntry: 'Feeling stressed about work, stomach hurts.' },
        ],
      },
      'sleep-stress-logger': {
        sleep: () => [
          { date: '2023-10-20', sleep: { bedtime: '23:00', wakeTime: '07:00', duration: 8, quality: 6, interruptions: 1, deepSleep: 2, remSleep: 1.5, lightSleep: 4.5 } },
        ],
        stress: () => [
          { date: '2023-10-20', stress: { level: 7, triggers: ['work', 'deadline'], copingStrategies: ['deep_breathing'], effectiveness: 6 } },
        ],
      },
    };
  ...
  private async storeCrossAppData(data: CrossAppData[]): Promise<void> {
    ...
    const uniqueData = newData.filter((item, index, self) => 
      index === self.findIndex(t => 
        ...
        t.sourceApp === item.sourceApp && 
        t.dataType === item.dataType
      )
    );
